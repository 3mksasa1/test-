...
client.on("message", async message => {
  ...
  if (message.channel.id != "ايدي الروم") return;    
  if(message.author.id === client.user.id) return;
      if (message.author.send) {
    message.channel.send({files: ["https://media.discordapp.net/attachments/1046359582643073105/1062723919871737907/20230106_152804.jpg"]});
...
client.on('message', msg => {
  if (msg.content === 'خط') {
    msg.delete();
    msg.reply({files: ["https://media.discordapp.net/attachments/1046359582643073105/1072968920870436915/20230208_225357.png"]});
...
client.on('message', message => {
    if (message.channel.id != '1062457671460073512') return;
...
client.on('message', message => {
    if (message.channel.id != '1062726149379207218') return;
    if (message.member.user.bot) return;
    message.react('👍') ;
}); 
...
client.on("message", async (message) => {
if(message.channel.id != "1015711332248588338") return //ايدي روم
if(message.author.send) {
...
client.on('message', msg => {
  if (msg.content === 'السلام عليكم') {
    msg.channel.send (':**__وعليكم السلام ورحمة الله وبركاته |    :1008234108259938374:__**');
  }
});
...
setInterval(() => {
  const channelid = "1077913783760072704" //ايدي الروم
  const channel = client.channels.cache.get(channelid);
  if (!channel) return
  channel.join()
}, 1000);
